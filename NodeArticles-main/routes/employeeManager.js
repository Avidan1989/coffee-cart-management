...
router.get("/requests", isAuthenticated, checkRole("admin"), (req, res) => {
  const sql = `
    SELECT er.id, er.user_id, u.username, u.first_name, u.last_name, er.from_date, er.to_date, er.reason, er.file_path, er.response
...
router.get(
  ...
  (req, res) => {
    ...
    const sql = `
    SELECT id, from_date, to_date, reason, file_path, response, created_at, status, manager_comment
